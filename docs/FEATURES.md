...
Detailed guide, type signatures, and practical code examples for every utility in `@kjangid/url-encode-tools`.
...
```ts
import { parseUrl, tryParseUrl, toUrlString, type ParsedUrl } from "@kjangid/url-encode-tools/url-parser";
```
...
```ts
import { parse, stringify, appendParams, removeParams } from "@kjangid/url-encode-tools/query-string";
```
...
```ts
// Serialization
const qs = stringify({ filter: { status: "active", tags: ["node", "ts"] } }, { arrayFormat: "bracket" });
...
} from "@kjangid/url-encode-tools/url-validator";
```
...
```ts
import { buildUtm, extractUtm, hasUtm } from "@kjangid/url-encode-tools/utm-builder";
```

### Example

```ts
const campaignUrl = buildUtm(
...
import {
  ...
  isBase64,
  isBase64Url,
} from "@kjangid/url-encode-tools/base64";
```
...
import {
  ...
  encodePath,
  isEncoded,
} from "@kjangid/url-encode-tools/url-encoder";
```
...
```ts
import { escapeHtml, escapeAttribute, unescapeHtml, stripHtml } from "@kjangid/url-encode-tools/html-encoder";
```
...
```ts
import { encode, decode, encodeBytes, decodeBytes, format, isHex } from "@kjangid/url-encode-tools/hex";
```
